...
public class CreateUserCommandHandler(
    ...
    IMemoryCache memoryCache) : IRequestHandler<CreateUserCommand, TokenDto?>
{
    ...
    public async Task<TokenDto?> Handle(CreateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        Log.Information("Inserting the user ID {UserId} in Cache with same key from user ID to futures validations of token.", user.UserId);
        _memoryCache.Set(user.UserId, user.UserId);
